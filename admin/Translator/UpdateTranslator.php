...
<body>
    <?php
        ...
        $result = $translatorObj->getTranslatorById($id);
        if (!$result) {
            die("<h3> Lỗi lấy dữ liệu từ database");
        }
        $translator = $translatorObj->data;
    ?>
    <form name="form1" method="post" action="UpdateTranslatorHandle.php" enctype="multipart/form-data">
        <input type="hidden" name="id" id="id" value="<?=$translator["translator_id"]?>">
        <table  border="0" align="center" cellpadding="5" cellspacing="0">
            <tr>
                <td width="155">ID</td>
                <td width="300"><p><?=$translator["translator_id"]?></p></td>
            </tr>
            <tr>
                <td width="155">Tên dịch giả</td>
                <td width="300"><input type="text" name="tTranslatorName" id="tTranslatorName" value="<?=$translator["translator_name"]?>"></td>
            </tr>
            <tr>
                <td width="155">Tiểu sử</td>
                <td width="300"><input type="text" name="tTranslatorBio" id="tTranslatorBio" value="<?=$translator["translator_name"]?>"></td>
            </tr>
